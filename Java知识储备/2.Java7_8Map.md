...
- 内部使用数组+单向链表构成
- 数组容量始终是$2^n$
- 可以存放key为null且存放在数组的0号位置
- 在未指定数组初始容量时，默认为

首先，我们用下面这张图来介绍 HashMap 的结构。

![](C:\Users\17646\Desktop\简历内容\Java知识储备\assets\Java7 HashMap的结构.png)
...
![](C:\Users\17646\Desktop\简历内容\Java知识储备\assets\Java7ConcurrentHashMap.png)
...
为了降低这部分的开销，在 Java8 中，当链表中的元素达到了 8 个时，会将链表转换为红黑树，在这些位置进行查找的时候可以降低时间复杂度为 **O(logN)**。HashMap中初始容量为16个。

- HashMap内部是一个Node<K,V>[] table数组 + 链表 + 红黑树。
- 在创建HashMap时不创建数组，在第一次插入数据时创建。

![](C:\Users\17646\Desktop\简历内容\Java知识储备\assets\Java8HashMap结构.png)
...
- 数组+链表+红黑树
- 如果当前数组中位置没有放置过元素，则使用CAS操作来修改这个位置的值
- 使用synchronized对数组中的Node进行同步操作
...
final V putVal(K key, V value, boolean onlyIfAbsent) {
    ...
    for (Node<K,V>[] tab = table;;) {
        ...
        // 找该 hash 值对应的数组下标，得到第一个节点 f
            ...
            //如果数组该位置为空，
            //用一次 CAS 操作将这个新值放入其中即可，这个put操作差不多就结束了，可以拉到最后面了
...
当sizeCtl < 0时，表示已经有线程开始初始化了。sizeCtl > 0表示的是数组中能容纳的容量，为$2^n $。
...
private final Node<K,V>[] initTable() {
    ...
    while ((tab = table) == null || tab.length == 0) {
        ...
        //SIZECTL代表的是 sizeCtl 的原始值，sc代表的是 sizeCtl 当前值，最终修改的是sizeCtl的值
...
在HashMap的基础上加入了链表数据结构。

## 五、HashTable

- 内部由 数组+链表 实现
- 在创建HashTable时，数据即创建了数组
- 当达到阈值threshold时即扩容，扩容为 $2*oldCapacity+1$
- put方法和get方法使用synchronized关键字修饰来保证线程安全
- value不能为null值
...
若容量达到阈值，则先扩容$ oldCapacity*2 + 1$ ，再插入值。

若key在HashTable中已经存在，则返回旧值，更新旧值；若不存在，则返回null。
...
public synchronized V put(K key, V value) {
    ...
    // 如果index处链表中有元素的entry.key和要插入的相等，则更新value并返回旧value
...
/**
* 如果数组中存储的数据容量达到了阈值时插入需要先扩容 oldCapacity*2 + 1；
* 根据Hash值计算出在数组中的index，然后将原有的Node构成的链作为当前新创建的Node的next
*/
...
方法使用synchronized关键字修饰实现同步。若没找到则返回null。
...
```
## 六、SortedMap的实现类TreeMap

- 内部是由 红黑树 实现的
- put函数返回的是这个节点插入的父节点（root节点的父节点为null）
