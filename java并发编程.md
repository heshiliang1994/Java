...
如果一个线程实例A执行了threadB.join()，其含义是：当前线程A会等待threadB线程终止之后threadA才会继续执行。Thread类除了提供join()方法外，另外还提供了超时等待的方法，如果线程threadB在等待的时间内还没有结束的话，threadA会在超时之后继续执行。可以看出来当前等待对象threadA会一直阻塞，直到被等待对象threadB结束后即isAlive()返回false的时候才会结束while循环，当threadB退出时会调用notifyAll()方法通知所有的等待线程。如：
...
> 个人理解：在调用start()之后，Thread的实例会和系统的线程关联，在主线程中调用join方法，当前Thread的实例会进入wait状态，等到线程即将退出时，线程自身会调用notifyAll，来唤醒join的线程来重新检测isAlive()
...
| 原子操作     | 作用内存         | 描述                                                         |
| ------------ | ---------------- | ------------------------------------------------------------ |
| lock(锁定)   | 主内存中的变量   | 它把一个变量标识为一个线程独占的状态                         |
| unlock(解锁) | 主内存中的变量   | 它把一个处于锁定状态的变量释放出来，释放后的变量才可以被其他线程锁定 |
| read(读取)   | 主内存中的变量   | 它把一个变量的值从主内存传输到线程的工作内存中，以便后面的load动作使用 |
| load(载入)   | 主内存中的变量   | 它把read操作从主内存中得到的变量值放入工作内存中的变量副本   |
| use(使用)    | 工作内存中的变量 | 它把工作内存中的一个变量值传递给执行引擎，每当虚拟机遇到一个需要使用到变量的值的字节码执行时将会执行这个操作 |
| assign(赋值) | 工作内存中的变量 | 它把一个从执行引擎接收到的值赋给工作内存的变量，每当虚拟机遇到一个给变量赋值的字节码指令时执行这个操作 |
| store(存储)  | 工作内存的变量   | 它把工作内存中一个变量的值传送给主内存中以便随后的write操作使用 |
| write(操作)  | 主内存的变量     | 它把store操作从工作内存中得到的变量的值放入主内存的变量中    |

>- 将变量从主存读取到工作内存，必须顺序执行 read、load
>- 将变量从工作内存同步回主存，必须顺序执行store、write
>- 不允许 read和load、store和write单独存在
>- 不允许一个线程丢弃它最近的一个assign操作，变量在工作内存被更改之后必须同步更改回主存
>- 不允许一个线程丢弃它最近的assign操作，变量在工作内存中改变之后必须把该变化同步回主内存
...
- ----> volatile读操作 ----> LoadLoad屏障 +  LoadStore屏障

> https://gee.cs.oswego.edu/dl/jmm/cookbook.html
>
> 为何在volatile写之前不加 StoreLoad屏障？
>
> - 对于「volatile读 -> LoadLoad屏障 + LoadStore屏障 -> volatile写」，在「volatile读」之后会有LoadStore屏障
> - 对于「普通读 -> volatile写」，无数据依赖，重排不会影响结果
>
> 为何volatile读之前不加StoreLoad/LoadLoad屏障？
>
> - volatile读 -> LoadLoad屏障 + LoadStore屏障 -> volatile读
> - 普通读 -> volatile读 -> LoadLoad屏障 + LoadStore屏障，读取操作之间没有相互依赖
